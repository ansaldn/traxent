...
| **R-02** | Over-broad deploy credentials. **Static keys are gone** — all four workflows assume an OIDC role (`traxent-github-deploy`), verified 2026-08-06. What remains is scope: several statements in that role still use `Resource: *` (ApiGw, CloudFront, WAF, SNS, CloudWatch, Budgets) (audit **H2**) | Medium | High | **Medium** | Tighten the remaining `Resource: *` statements to `traxent-*`. DynamoDB, Lambda, Logs, SSM, IAM and S3 are already scoped; the DynamoDB scope was widened to `table/Traxent*` on 2026-08-06 so new tables deploy without an IAM edit. | David | **Mitigated** |
...
| 2026-06-29 | David Ansah | R-08: entity name reconciled to **Akpan Holdings Limited** across `privacy.html`, `terms.html`, `security.txt` and `security.html` (code change). ICO registration + Companies House number/address confirmation remain open; R-08 stays open. |
